              ...
              <div class="flex items-center">
                <div class="flex-shrink-0">
                  <a href="/">
                    <img
                      class="h-12"
                      src="../images/withnetworks_logo.png"
                      alt="withnetworks"
                    />
                  </a>
                </div>
...
const navigation = ref([
  { name: 'MAINTEST', href: '#', current: true },
  { name: '자산', href: '/property', current: false },
  { name: 'FAQ', href: '/faq', current: false },
  { name: '주간보고', href: '/weeklyreport', current: false },
  { name: '문의하기', href: '/question', current: false }
]);
const user = {
  name: 'ADMIN',
  email: 'admin@withnetworks.com',
  imageUrl:
...
onMounted(() => {
  ...
  if (storedNavigation) {
    navigation.value = JSON.parse(storedNavigation);
  }
});

const toggleCurrent = (selectedItem: any) => {
...
const currentTabName = computed(() => {
  const activeTab = navigation.value.find((item) => item.current);
